...
host <- "https://data.chc.ucsb.edu/experimental/CHIRTS-ERA5/tmax/tifs/daily"
...
files %>% 
  map(.progress = TRUE, ~{
    global <- .x %>% file.path("/vsicurl", host, .) %>% rast()
  ...
  iwalk(~{
    ...
    .x %>% 
      ...
      mask(border) %>% 
      writeRaster(
        here("data/chirts/raw", paste0(.y, ".tif")), 
        overwrite = TRUE
      )
  })
...
bboxes %>% 
  iwalk(.progress = TRUE, ~{
    raw <- here("data/chirts/raw", paste0(.y, ".tif")) %>% rast()
    out <- tibble(days = names(raw)) %>% 
      ...
      group_by(year, month) %>%
      summarise(dat = raw[[days]] %>% max() %>% list())
    names <- make_date(out$year, out$month)
    out <- out$dat %>% rast()
    names(out) <- names
    out %>% writeRaster(here("data/chirts", paste0(.y, ".tif")))
  })